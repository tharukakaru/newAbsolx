...
function BrandValue({ brand, value }) {
  if (!brand) {
    return value;
  }
...
export default function SentinelAircraftContent({
  titleId,
  name,
  typeLabel,
  year,
  ...
  const updateDescLineHeight = useCallback(() => {
    ...
    const specsNode = specsRef.current;
    const desc = descRef.current;

    if (!content || !specsNode || !desc) {
      return;
    }

    const specsBottom = specsNode.getBoundingClientRect().bottom;
    ...
    <div className="megha-aircraft-section">
      ...
      >
        <div className="x-striker-profile">
          ...
          >
            <h2 id={titleId} className="x-striker-aircraft-stage__name">
              <span>{name}</span>
              {typeLabel ? (
                ...
                return (
                  ...
                  >
                    <dd className="x-striker-specs__value x-striker-specs__value--megha">
                      <BrandValue brand={row.brand} value={row.value} />
                    </dd>
                  </div>
                );
              }
