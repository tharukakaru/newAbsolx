...
export default class BlackHole extends Model {
  ...
  uniforms = {
    ...
    rampCol1: uniform(color(0.65, 0.8, 1.0)),
    rampPos1: uniform(float(0.07)),
    rampCol2: uniform(color(0.75, 0.22, 0.08)),
    rampPos2: uniform(float(0.34)),
    rampCol3: uniform(color(0.04, 0.01, 0.01)),
    rampPos3: uniform(float(1.0)),

    rampEmission: uniform(float(2.25)),
    emissionColor: uniform(color(0.0, 0.01, 0.03)),
    ...
    material.colorNode = Fn(() => {
      ...
      Loop(iterCount, () => {
        ...
        const hotInnerMask = smoothRange(
          ...
          originRadius.mul(2.0),
          1.0,
          0.0
        ).mul(diskMask);
        const cyanRimMask = smoothRange(
          xyLen,
          originRadius.mul(0.85),
          outerRadius.mul(0.6),
          1.0,
          0.0
        );
        ...
        const hotWhite = vec3(0.7, 0.85, 1.0).mul(hotInnerMask.mul(0.9));
        const blueRim = vec3(0.3, 0.6, 1.0).mul(cyanRimMask.mul(0.85));
        const redAccent = vec3(0.95, 0.05, 0.02).mul(outerWarmMask.mul(1.1));
        const warmAccent = vec3(1.0, 0.25, 0.05).mul(
          outerWarmMask.mul(streakMask).mul(0.3)
        );
